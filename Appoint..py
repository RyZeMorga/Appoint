...
ver = Label(master, text="Version 1.6.4", font=("Arial", 12), bg="black", fg="DarkOrange1")
...
def openNewWindow2():
   ...
   # This is the function that gets the records from the database and outputs them into the new window
    def query():
       conn = sqlite3.connect("Appoint.db")
       c = conn.cursor()
       ...
       print(records) # This creates a command that you can call into a label so it outputs the records to it

      # Label that contains the records
       ...
       print("Query success") # Prints to the terminal to show that the query was a success
       
       
       

       conn.commit() # Apply all changes that were made into the database
       conn.close() # Close connection with the database

    query() # Call the function so that it actually displays the records
